...
def index():
    if request.method == 'POST':
        ...
        if not stock:
            stock = 'POWERGRID.NS','ONGC.NS','AAPL.NS' # Default stock if none is entered
        
        # Define the start and end dates for stock data
        start = dt.datetime(2021, 1, 1)
        end = dt.datetime(2025, 5, 19)
    ...
    return render_template('index.html')
# Instead of calling Tkinter code in a thread:
import threading
...
# WRONG: Tkinter should run in main thread
threading.Thread(target=gui_thread).start()

# RIGHT: use `after()` to run periodic updates instead
